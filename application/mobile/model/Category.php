...
class Category extends Model{
   ...
   public function makeTree(){
        ...
        foreach ( $category as $k => $v ) {
			...
			$category [$k] ['brand']=db('brand')->where($condition2)->limit(10)->order("id desc")->select();  
			
		   /**分类商品**/
			$category [$k] ['doc'] = array ( );
			$map['category_id']=array("in",$cid);
			$map['status']=1;
			$category [$k]['item'] =Db::name('goods')->where($map)->order("id desc")->limit(6)->select();   
    ...
    public function getDatalist($cateid){     
		...
		$map['status']=1;   
		$list=Db::name( 'goods' )->where($map)->limit(10)->order("id desc")->select();         
		if($list){
			foreach ($list as $k => $v ) {
			   /**重组数据**/
				$id=$v["id"];
    ...
    public function getCategory(){
		$field = 'id,pid,title,cover_id';
		$condition['display']=1;
		$condition['status']=1;
		
		$category = $this->field($field)->order('id asc,sort desc')->where($condition)->select( );
