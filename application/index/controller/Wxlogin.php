...
class Wxlogin extends Home{
	...
	//微信认证链接
       public function index(){
			...
			$code_url="https://open.weixin.qq.com/connect/qrconnect?appid=wx603bc2ff08dba3bc&redirect_uri="
			.$code_url."&response_type=code&scope=snsapi_login&state=mahuateng&connect_redirect=1#wechat_redirect";
            //  echo $url;
       ...
       public function callback(){        
           ...
           $url ='https://api.weixin.qq.com/sns/oauth2/access_token?appid=' . $appid. '&secret=' . $secret . '&code=' . $code . '&grant_type=authorization_code'; 
		    ...
		    if (empty ( $access ['access_token'] )) {
			   $this->error ( '获取access_token失败,请确认AppId和Secret配置是否正确,然后再重试。'.$info ['appid'].$info ['secret'] );
		    }else{
			   ...
			   // 拉取用户信息(需scope为 snsapi_userinfo)
                ...
                $uid= $this->get_uid_by_ucenter($openid,$appid,$data["nickname"]);
				if(0 < $uid){ //UC登录成功
						/* 登录用户 */
     ...
     public function get_uid_by_ucenter($openid,$appid,$nickname) {
		...
		$info ['openid'] = $openid;
		$info ['token'] =  $appid;
		$res = db ( 'thirdLogin' )->where ( $info )->find ();
		if ($res){
			return $res ['uid'];
		}
		else {
             ...
             $res=db ( 'thirdLogin' )->insertGetId($info);
			return $res?$uid:0;
		}
	}
