...
class Cart extends Model{
	...
	public  function additem() {
		...
		if($item) {
			...
			$array['path'] = $path;
			$array['specifications'] = $specifications;
			$exsit="0";$url=url('Goods/detail?id='.$goodsid);
			$img=get_cover($cover_id,'path');
			$title=get_goods($goodsid,'title');
			$tpl6="<li class='item{$id}'><a href=".url('Goods/detail?id='.$goodsid)."
			...
			$tpl='<li class="cart_item item'.$id.'"><div class="cart_item_pic"><a href="'.$url.'"><img width="85" height="85" src="'.$img.'"></a></div><div class="cart_item_desc"><a href="'.$url.'" class="cart_item_name">'.$title.'</a><div class="cart_item_sku"><span>规格：'.$specifications.'</span></div><div class="cart_item_price"><span class="cart_price">￥'.$price.'*<em class="num'.$id.'">'.$array['num'].'</em></span></div></div>	
	...
	public  function addList() {
		...
		if($item) {
			...
			{$price}</em>×<em class='item_num_{$id}'>{$array['num']}</em></span>
				...
				$tpl='<li class="cart_item item'.$id.'"><div class="cart_item_pic"><a href="'.$url.'"><img width="85" height="85" src="'.$img.'"></a></div><div class="cart_item_desc"><a href="'.$url.'" class="cart_item_name">'.$title.'</a><div class="cart_item_sku"><span>规格：'.$specifications.'</span></div><div class="cart_item_price"><span class="cart_price">￥'.$price."<em class='num{$id}'>{$array['num']}</em></span></div></div></li>";
		}
		session($key,$array);
	//登录用户，处理详情页ajaxt提交的数据保存到数据库
		if(is_login()){
