				...
				FileUploaded: function(up, file, data) { 
				 if(upload_type==1){
					...
					if(data.status == 200){  
						...
						$("#"+field).parents(".form-item").find('.uploader_preview').html(
				...
				FileUploaded: function(up, file, data) { 
				 if(upload_type==1){
					...
					if(data.status == 200){  
						...
						$("#"+field).parents(".form-item").find(".ipt").val(src);
						$("#"+field).parents(".grzl_con01_t").find('img').attr("src",src);
				 ...
				 if(upload_type==1){
					...
					if(data.status == 200){  
						...
						if(upload_type==1){
							   ...
							   $("#"+field).parents(".form-item").find(".uploader_preview").append(html);
                               setinput(field);  	   
						}
...
function delimage(obj,field){ //删除
 ...
 function setinput(field){ //取值
		    var option =$("#"+field).parents(".form-item").find(".uploader_preview img");
			...
			init: {
				...
				FileUploaded: function(up, file, data) { 
				 if(upload_type==1){
					...
					if(data.status == 200){  
						$("#"+field).parents(".form-item").find(".ipt").val(src);
						$("#"+field).parents(".form-item").find('video').attr("src",src);
						if(upload_type==1){
							   Setvideo(filed,src);
						}else{    
							  $("input[name='"+field+"']").val(data.id);
