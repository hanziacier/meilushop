    ...
    public function build($url = '', $vars = '', $suffix = true, $domain = false)
    {
       ...
       // URL组装
		...
		if($module=="index"){
			...
			$matches=array();
			//没有找到入口文件
			if($matches&&!strpos($route,$matches[0])){
				   if(!config("app_rewrite")){
					  $extend=$matches[0]."/";
				   }
			}
        ...
        // URL组装
        $url = $domain . rtrim($this->root ?: $this->app['request']->root(), '/'). '/'.$extend . ltrim($url, '/');
        if(strpos($url,"index.php")){
			 $count=substr_count($url,"index.php");
		     if($count===2){
